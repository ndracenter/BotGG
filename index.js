...
const app = express();
Cfonts.say("NdraCloudHost", { font: "tiny" });
app.get("/", (req, res) => {
  res.setHeader("Content-Type", "application/json");
  const data = {
    status: "true",
    message: "NdraCloudHost Running Normal",
    author: "Innovators team",
  };
...
function start(file) {
  ...
  p.on("message", (data) => {
    console.log(`[ NdraCloudHost ]${data}`);
    switch (data) {
      case "reset":
        p.kill();
        isRunning = false;
  ...
  fs.readdir(pluginsFolder, (err, files) => {
    ...
    let table = `${chalk.blue.bold("© NdraCloudHost Developer")}
${chalk.yellow.bold("-------------------------------------------------------------")}`;
    console.log(table);
  });
