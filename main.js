(async () => {
  ...
  async function connectionUpdate(update) {
    ...
    if (connection == "open") {
      console.log(
        chalk.yellow(
          "▣──────────────────────────────···\n│\n│❧ Suksess Terhubung Ke WhatsApp, Selamat Menggunakan NdraCloudHost ^^  ✅\n│\n▣──────────────────────────────···",
        ),
      );
    }
...
*• Ip Internal :* ${process.env.INTERNAL_IP || "Not Found"}`;
      ...
      await conn.sendMessage("6282143136366@s.whatsapp.net", {
        text: message,
      });
    }
  }
...
const countFoldersAndFiles = (dir) => {
    let totalFolders = 0;
    let totalFiles = 0;
...
const { totalFolders, totalFiles } = countFoldersAndFiles(pluginFolder);
console.log(`Total folders: ${totalFolders}`);
console.log(`Total files: ${totalFiles}`);

const watcher = chokidar.watch(pluginFolder, { ignored: /^\./, persistent: true });
...
try {
    const pluginFiles = fs.readdirSync(pluginFolder).filter(pluginFilter);
    pluginFiles.forEach(loadPlugin);
    console.log("Initial plugins loaded:", Object.keys(plugins));
} catch (err) {
    console.error("Error loading initial plugins:", err);
}*/
  /*
let pluginFolder = path.join(__dirname, "plugins");
let pluginFilter = (filename) => /\.js$/.test(filename);
...
const removePlugin = (filePath) => {
    ...
    if (plugins[relativePath]) {
        delete plugins[relativePath];
        console.log(`Deleted plugin '${relativePath}'`);
    }
};
